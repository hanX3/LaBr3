...
Sort::Sort(const std::string &filename_in, const std::string &filename_out)
{
  ...
  tr_out = new TTree("tr", "DT5720_DPP_PSD");
...
void Sort::Process()
{
  ...
  for(Long64_t i=0;i<total_entry;i++){
    ...
    d.channel = channel;
    d.energy = par[channel][0]+par[channel][1]*(Double_t)energy+par[channel][2]*(Double_t)(energy*energy);
    d.timestamp = timestamp+ts_offset[tr_ch];
