...
int main(int argc, char *argv[])
{
  ...
  int run = atoi(argv[1]);
  TString file_in = TString::Format("../../data/data_R%04d.root", run);
  std::cout << "analysis " << file_in << std::endl;
  
  int entry_start = atoi(argv[2]);
  int entry_stop = atoi(argv[3]);
  TString file_out = TString::Format("../../rootfile/data_R%04d_%09d_%09d_ana.root", run, entry_start, entry_stop);
